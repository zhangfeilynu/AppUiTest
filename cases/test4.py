...
# 安装app
async def install_app(device_name):
    try:
        ...
        detail.setdefault('device_name', device_name)
        detail.setdefault('step_id', 0) #0对应安装APP
        detail.setdefault('start_time', int(round(time.time() * 1000)))
        cmd = ''.join(['adb  -s ', device_name, ' install ', apk_path])  # type: str
...
async def install_app_3(device_name):
    await install_app(device_name)
...
if __name__ == '__main__':
    ...
    tasks = [install_app_3(device_name) for device_name in devlist]
